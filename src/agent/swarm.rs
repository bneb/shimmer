...
use anyhow::Result;
use llama_cpp_2::{llama_backend::LlamaBackend, llama_batch::LlamaBatch, model::AddBos};

use super::state::{AgentConfig, AgentState, BenchmarkResult, SwarmState};
use super::state::{BATCH_SIZE, COMPACT_THRESH, CTX_SIZE, MAX_TOKEN_BYTES, MAX_TOKENS};
...
impl super::Agent {
    ...
    fn init_swarm_decode(
        ...
        for (i, a) in agents.iter().enumerate() {
            groups.entry(a.lora_path.clone()).or_default().push(i);
        }
                ...
                for (j, &t) in tokens.iter().enumerate() {
                    ...
                    if batch.n_tokens() == BATCH_SIZE as i32 || is_last {
                        ...
                        if is_last {
                            ...
                            if let Some(sampler) = &mut a.sampler {
                                sampler.accept(m_i);
                            }
    ...
    fn set_swarm_lora_adapter(
            ...
            && let Some(adapter) = agents[idx].lora_adapter.as_mut()
        {
            ...
            if let Some(dummy_ptr) = dummy {
                let _ = ctx.lora_adapter_remove(unsafe { &mut *dummy_ptr });
            }
        }
        Ok(())
    }
    ...
    pub(crate) fn sample_token_for_agent(
        ...
        let m_i = if let Some(sampler) = &mut a.sampler {
            ...
            ctx.candidates_ith(a.last_batch_idx)
                .max_by(|a, b| {
                    a.logit().partial_cmp(&b.logit()).unwrap_or(std::cmp::Ordering::Equal)
                })
                .map(|d| d.id())
    ...
    fn swarm_generation_loop(&self, state: &mut SwarmState, config: &AgentConfig) -> Result<()> {
        let mut interceptors: Vec<_> = (0..state.agents.len())
            .map(|_| ToolInterceptor::new(config.enable_time_travel, true))
            .collect();
        loop {
            if state.agents.iter().all(|a| !a.active) {
                break;
            }
    ...
    pub fn enforce_swarm_compaction(&self, state: &mut SwarmState) -> Result<()> {
        let global_used: usize =
            state.agents.iter().filter(|a| a.active).map(|a| a.n_cur as usize).sum();
    ...
    pub fn generate_swarm_step(&self, state: &mut SwarmState) -> Result<()> {
        ...
        for (lora_path, agent_indices) in groups {
            ...
            state.batch.clear();
            let batch_idx =
                self.prepare_swarm_step_batch(&mut state.batch, &mut state.agents, &agent_indices)?;
            if batch_idx > 0 {
    ...
    fn prepare_swarm_step_batch(
        ...
        for &idx in agent_indices {
            let a = &mut agents[idx];
            let Some(t) = a.pending_token else {
                continue;
            };
            a.history.push(t);
    ...
    pub fn process_swarm_tools(
        &self,
        state: &mut SwarmState,
        interceptors: &mut [ToolInterceptor],
    ) -> Result<()> {
        ...
        for (i, a) in state.agents.iter_mut().enumerate() {
            ...
            let Some(last_t) = a.history.last().copied() else {
                continue;
            };
            let t_bytes = self
                .main_model
                .token_to_piece_bytes(last_t, MAX_TOKEN_BYTES, true, None)
                .unwrap_or_default();
            let token_str = String::from_utf8_lossy(&t_bytes).to_string();
            if interceptors[i].feed_token(&token_str)
                && let Some(call) = interceptors[i].detected_call.take()
            {
                tools_to_run.push((i, call, a.n_cur));
            }
        }
        for (i, call, pos) in tools_to_run {
            self.execute_swarm_tool(state, i, call.0, call.1, pos)?;
        }
        Ok(())
    }

    fn execute_swarm_tool(
        &self,
        state: &mut SwarmState,
        agent_idx: usize,
        pat: String,
        ...
        for (j, &t) in obs_tokens.iter().enumerate() {
            ...
            if state.batch.n_tokens() == BATCH_SIZE as i32 || is_last {
                state.ctx.decode(&mut state.batch)?;
                if is_last {
                    a.last_batch_idx = state.batch.n_tokens() - 1;
                }
                if !is_last {
                    state.batch.clear();
                }
        ...
        state
            ...
            .map(|a| {
                ...
                BenchmarkResult {
                    ...
                    token_count: a.total_generated,
                    duration_secs: duration,
                    tps: a.total_generated as f64 / duration,
                }
            })
            .collect()
    }
    ...
    pub fn should_continue_swe_bench(
        ...
        if edits_started > 1 && edits_ended < edits_started {
            return Some(crate::models::format_system_nudge(
                model,
                "You have not provided the closing </edit> tag. Please finish the edit block. Do \
                 not stop.",
            ));
        }
        ...
        if tool_calls >= 6 && edits_started <= 1 {
            return Some(crate::models::format_system_nudge(
                model,
                "You have enough information. Provide your fix now. Use either XML or JSON \
                 format. Do not use any more tools.",
            ));
        }
        ...
        if edits_started <= 1 && history_str.contains("CRITICAL: If a search tool yields no output")
        {
            if last_output_tokens == 0 {
                return Some(crate::models::format_system_nudge(
                    model,
                    "Your last response was empty or incorrectly formatted. You MUST either \
                     execute a JSON tool call or provide an <edit> block.",
                ));
            } else {
                return Some(crate::models::format_system_nudge(
                    model,
                    "Investigate using tools (rg, cat, fd). Find the relevant code, then provide \
                     your fix (XML or JSON format).",
                ));
            }
        }
