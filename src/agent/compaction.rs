...
impl super::Agent {
    ...
    pub(crate) fn enforce_compaction(
        ...
        if let Some((start, end)) = state.compactor.compact(&mut state.history, 0) {
            ...
            state
                .ctx
                .kv_cache_seq_add(seq_id as i32, Some(end as u32), None, -(shift_amount as i32))
    ...
    fn summarize_if_tool_output(&self, state: &mut EngineState) {
        ...
        let text: String = tokens
            .iter()
            .filter_map(|t| self.main_model.token_to_piece_bytes(*t, 256, true, None).ok())
        ...
        let summary_tokens = self
            .main_model
            .str_to_token(&summary, llama_cpp_2::model::AddBos::Never)
            .unwrap_or_default();
        if summary_tokens.len() >= tokens.len() {
            return;
        }
        ...
        if diff > 0 {
            ...
            let _ = state.ctx.kv_cache_seq_add(0_i32, Some(page_end as u32), None, -(diff as i32));
        }
    }
    ...
    pub fn extract_high_entropy_summary(text: &str) -> String {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            if keywords.iter().any(|&k| s.contains(k)) {
                for j in i.saturating_sub(3)..=(i + 3).min(n.saturating_sub(1)) {
                    kept.insert(j);
                }
            }
